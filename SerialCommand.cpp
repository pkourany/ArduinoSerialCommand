...
void SerialCommand::readSerial() 
{
	...
	#else
	while ((usingSoftwareSerial==0 && commandSerial.available() > 0) || (usingSoftwareSerial==1 && SoftSerial->available() > 0) )
	#endif
	{
		int i; 
		boolean matched; 
		if (usingSoftwareSerial==0) {
			...
			inChar=commandSerial.read();   // Read single available character, there may be more waiting
		} else {
			#ifndef SERIALCOMMAND_HARDWAREONLY
			// SoftwareSerial port
		...
		debugSerial.print(inChar);   // Echo back to serial stream
		#endif
		if (inChar==term) {     // Check for the terminator (default '\r') meaning end of command
			...
			for (i=0; i<numCommand; i++) {
				#ifdef SERIALCOMMANDDEBUG
				debugSerial.print("Comparing ["); 
				debugSerial.print(token); 
				debugSerial.print("] to [");
				debugSerial.print(CommandList[i].command);
				debugSerial.println("]");
				#endif
				...
				{
					#ifdef SERIALCOMMANDDEBUG
					debugSerial.print("Matched Command: "); 
...
void SerialCommand::addCommand(const char *command, void (*function)())
{
	if (numCommand < MAXSERIALCOMMANDS) {
		#ifdef SERIALCOMMANDDEBUG
		debugSerial.print(numCommand); 
		debugSerial.print("-"); 
		debugSerial.print("Adding command for "); 
		...
		// the command
		#ifdef SERIALCOMMANDDEBUG
		debugSerial.println("Too many handlers - recompile changing MAXSERIALCOMMANDS"); 
		#endif 
	}
}
